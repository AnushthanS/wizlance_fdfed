...
const ResultTemplate = () => {
  ...
  useEffect(() => {
    axios
      .post("/api/subcategory", { subCategoryId })
      .then((res) => {
        console.log(res.data);
        setSubCategory(res.data.subCategory);
      })
      .catch((err) => console.log(err, "sub error"));
  }, []);
        ...
        <section className="text-gray-600 body-font">
          <div className="container px-5 py-24 mx-auto">
            <div className="flex flex-wrap justify-evenly -m-4">
              {gigs.length === 0 ? (
                <h1 className="text-3xl text-gray-800 font-bold">- No gigs found</h1>
              ) : (
                gigs.map((gig) => (
                  <div
                    key={gig.name}
                    className="lg:w-1/4 md:w-1/3 p-4 w-2/3 border-2 m-2 rounded-lg"
                  >
                    {loggedIn ? (
                      <Link
                        className="block relative h-52 rounded overflow-hidden"
                        to={`/gigdetails/${gig._id}`}
                      >
                        <img
                          alt="ecommerce"
